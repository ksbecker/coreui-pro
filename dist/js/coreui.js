...
(function (global, factory) {
  ...
  const EVENT_CLICK_DATA_API$9 = `click${EVENT_KEY$e}${DATA_API_KEY$9}`;
  /**
   * Class definition
   */
  ...
  const EVENT_KEYDOWN$2 = `keydown${EVENT_KEY$c}`;
  ...
  const EVENT_LOAD_DATA_API$6 = `load${EVENT_KEY$c}${DATA_API_KEY$8}`;
  ...
  const EVENT_SHOW$8 = `show${EVENT_KEY$b}`;
  const EVENT_SHOWN$8 = `shown${EVENT_KEY$b}`;
  const EVENT_HIDE$8 = `hide${EVENT_KEY$b}`;
  const EVENT_HIDDEN$8 = `hidden${EVENT_KEY$b}`;
  ...
  const EVENT_KEYUP_DATA_API$1 = `keyup${EVENT_KEY$a}${DATA_API_KEY$6}`;
